#!/usr/bin/env bash
# Source this file from helpers to keep supported tool state inside household.
...
export CONDA_ENVS_PATH="$HOUSEHOLD_ROOT/envs"
export CONDA_PKGS_DIRS="$HOUSEHOLD_ROOT/.conda/pkgs"
...
export MINIFORGE_PREFIX="$HOUSEHOLD_ROOT/.tools/miniforge3"
...
mkdir -p "$HOUSEHOLD_ROOT/.downloads" "$HOUSEHOLD_ROOT/.state" "$HOUSEHOLD_ROOT/records/runtime"
