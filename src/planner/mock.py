...
from planner.base import HouseholdPlanner


class MockPlanner(HouseholdPlanner):
    ...
    def infer(self, obs: Mapping[str, Any]) -> dict[str, np.ndarray]:
        ...
        joint_phase = np.float32(0.11) * np.arange(self._action_dim, dtype=np.float32)
        actions = self._amplitude * np.sin(time[:, None] + joint_phase[None, :])
